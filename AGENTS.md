...
이 프로젝트는 AI 개발자 성장 코치 서비스다. 사용자의 현재 역량, 목표 직무, GitHub 활동, 코딩테스트 이력을 바탕으로 부족 역량을 진단하고 주차별 학습 로드맵을 제공한다.

핵심 흐름은 다음과 같다.

1. 목표 직무, 현재 수준, 기술 스택 입력
2. 직무별 필요 역량 기준표 기반 역량 진단
3. GitHub 연결 및 실제 사용 기술 분석
4. 자기기입 스택과 실제 사용 스택 비교
5. 코딩테스트 풀이 이력 기반 약점 분석
6. 진단, GitHub 분석, 코테 분석 결과를 종합한 학습 로드맵 생성
7. 결과 저장, 재조회, 진도 체크
...
- 진단, GitHub 분석, 코딩테스트 분석, 로드맵은 결과 이력 관리를 위해 `version`을 유지한다.
...
- Coding Test Analysis: 유형별 정답률, 풀이 시간, 재시도, 취약 유형
